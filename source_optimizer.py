...
from utils import cut_page, url_to_domain
...
class ResourceOptimizer:
    ...
    def __init__(self, config, model=None, source_pool=None, evidence_model=None, validator=None):
        ...
        if  self.config.optimizer not in ['self_critical', 'online', 'offline']:
        ...
        # 选择证据发现方法 English vesion: evidence finder
        self.evidence_model = evidence_model
        # 资源池 English vesion: source pool
    ...
    def self_critical(self, query, negative_source_list, source_list):
        ...
        for negative_source in negative_source_list:
            ...
            if self.config.online_check_after_self_critical:
                ...
                updated_web.append(updated_source[-1])
                updated_url.append(url)
                # updated_evidence['hrefs'].append([])
                # updated_evidence['evidences'].append([])
    ...
    def offline_search(self, query, negative_source_list, source_list):
        # search the most similar query in query_web
        updated_web_list, updated_url_list = [], []
        updated_evidence = {"hrefs": [], "evidences": []}

        # use mongodb to GET query_web data
        # query_web_collection = self.source_pool.get_query_web_collection()
        # similar_data = query_web_collection.find({'$text': {'$search': query}}).sort([('query', 1)]).limit(1)
        ...
        query_web_data = self.source_pool.get_query_web_data()
        query_list = [line['query'] for line in query_web_data]
        ...
        if float(score[0]) > 0.5:
            ...
            similar_source = query_web_data[query_list.index(similar_query)]["web-urls"]
