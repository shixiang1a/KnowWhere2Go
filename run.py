...
from source_validator import ResourceValidator
from source_optimizer import ResourceOptimizer
...
def main(
    ...
    source_pool = ResourcePool(query_web_file_path="/home/llmtrainer/KW2G/result/query_web.json") 

    source_validator = ResourceValidator(config, source_generator.model)
    source_optimizer = ResourceOptimizer(config, source_generator.model, source_pool, None, source_validator)
    ...
    def web_finder(
        ...
        **kwargs,
        ...
        url_list, web_url = source_generator.url_recognition(web_show)
        query_web_url_map = source_generator.get_query_web_url_map(query_web, web_url)

        url_evidence_show = ""
        evidence_score_dict = {}
        if config.validator == "forbid":
            for no, expanded_query in enumerate(query_web_url_map.keys()):
                for web, url in query_web_url_map[expanded_query].items():
                    if [web, "<" + url + ">", "🟨Wait for check"] not in source_dataframe_show:
        ...
        if config.generate_answer and len(evidence_score_dict) > 0:
            ...
            ued_for_answer_generate = []
            length_for_answer_generate = 0
            for evidence, score in sorted_evidence_score_dict:
                if length_for_answer_generate + len(evidence) < 1000:
                    ued_for_answer_generate.append(evidence)
                    length_for_answer_generate += len(evidence)
                else:
                    break
            answer_show = source_validator.generate_answer(query, ued_for_answer_generate)
        else:
    ...
    gr.Interface(
        ...
        inputs=[
            ...
            gr.Radio(["webfilter", "contriver"], label="Filter", info="Which filter to use", value="webfilter"),
            gr.Radio(["self_critical", "online", "offline", "forbid"], label="Optimizer", info="Whether to use optimizer", value="forbid"),
            gr.Checkbox(label="Check Only", info="Whether to check only", value=True),
        ...
        title="KW2G",
        description="KW2G is an intelligent source retrieval system developed on the basis of a 7B-parameter LLaMA model finetuned to follow instructions.",  # noqa: E501
